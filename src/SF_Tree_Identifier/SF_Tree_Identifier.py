...
def main(
    user_input: str, check_nearby: bool = True
) -> pd.DataFrame | dict:
...
def create_output_dict(results: pd.DataFrame) -> list[dict]:
    ...
    for address in queried_addresses:
        ...
        grouped_results[['scientific_name', 'common_name']] = grouped_results.qSpecies.str.split('::', expand=True).fillna('')
        grouped_results = grouped_results.drop('qSpecies', axis=1).loc[:,['scientific_name', 'common_name']].apply(lambda name: name.str.strip())
